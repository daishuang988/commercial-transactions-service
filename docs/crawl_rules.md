...
- 订单关联的树外卖家商品一并导入，供订单详情显示（LEFT JOIN 不筛 is_show），状态处理问用户（本次按用户要求置 status=1 已售）
- **不要按"新系统语义"去改老商品状态**：老系统商品 status=0 即使已有完成订单也是正常的寄卖中状态（老系统不翻转商品状态），2026-08-14 曾把 20,929 件"被订单引用但 status=0"的商品改成已售，用户指出改反了，已还原——改老数据状态前必须先问用户
...
2. **老系统商品状态原样保留**：status=0 的商品即使被完成订单引用也是正常寄卖中（老系统机制如此，不是数据错误），曾误判为"老数据不一致"并批量改成已售、被用户纠正还原。**改老数据状态前必须先问用户**
3. **防 23:59 重复结算**：老系统每天 23:59 结算，新系统也是——已完成订单导入时 coupon_settled=1，否则新系统当晚给买家重发一遍"今日收益"。
4. **已售判定代码对齐**：老系统"已售=有订单"（卖完 status 仍是0），新系统原来只看 status 字段 → 已在代码层对齐（`repository.MerchSoldSub`：存在未取消订单即已售）。C端寄卖池/详情/购买/抢购全部排除已售，管理端列表状态按同规则换算，老 status 字段原样不动。验证口径：池未售 496、管理端待售 496 / 已售 36,512。
